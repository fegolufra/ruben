...
async function initDatabase() {
  ...
  if (!existingAdmin) {
    ...
    const hashR = bcrypt.hashSync('ruben123', 10);
    db.prepare("INSERT INTO users (username, password, full_name, role) VALUES (?, ?, ?, ?)").run('ruben', hashR, 'Ruben', 'admin');
    const hashJ = bcrypt.hashSync('jorge123', 10);
    db.prepare("INSERT INTO users (username, password, full_name, role) VALUES (?, ?, ?, ?)").run('jorge', hashJ, 'Jorge', 'vendedor');
    saveDb();
  } else {
    db.prepare("UPDATE users SET role = 'admin' WHERE username = 'ruben'").run();
    db.prepare("UPDATE users SET role = 'vendedor' WHERE username = 'jorge'").run();
  }
}
